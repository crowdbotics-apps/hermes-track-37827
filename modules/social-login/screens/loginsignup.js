...
import { unwrapResult } from "@reduxjs/toolkit"; // Custom Text Input
...
const SocialButtonsView = props => <View>
    ...
    <GoogleSigninButton onPress={props.onGoogleConnect} size={GoogleSigninButton.Size.Wide} color={GoogleSigninButton.Color.Dark} disabled={props.loading} style={_styles.ZHhBrYsi} />
    {(Platform.OS === "ios" || appleAuthAndroid.isSupported) && <AppleButton onPress={props.onAppleConnect} buttonStyle={AppleButton.Style.WHITE_OUTLINE} buttonType={AppleButton.Type.SIGN_IN} style={_styles.lsAJyDiA} />}
  </View>;
...
export const SignupTab = ({
  ...
  const onSignupPress = async () => {
    ...
    dispatch(signupRequest({
      ...
      Alert.alert("Signup Success", "Registration Successful. A confirmation will be sent to your e-mail address.");
    }).catch(err => console.log(err.message));
  };

  return <KeyboardAvoidingView>
      <View style={_styles.yUNWsXZV}>
      ...
      <Button title="Sign Up" loading={api.loading === "pending"} onPress={onSignupPress} />
      <SocialButtonsView loading={api.loading === "pending"} onFacebookConnect={() => onFacebookConnect(dispatch, navigation)} onGoogleConnect={() => onGoogleConnect(dispatch, navigation)} onAppleConnect={() => onAppleConnect(dispatch, navigation)} />
